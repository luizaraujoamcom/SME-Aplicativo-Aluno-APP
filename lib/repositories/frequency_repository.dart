...
class FrequencyRepository implements IFrequencyRepository {
  ...
  Future<CurricularComponent> fetchCurricularComponent(
    ...
    try {
      final response = await http.get(
          "${ApiUtil.HOST}/Aluno/frequencia/componente-curricular?AnoLetivo=$anoLetivo&CodigoUe=$codigoUE&CodigoTurma=$codigoTurma&CodigoAluno=$codigoAluno&CodigoComponenteCurricular=$codigoComponenteCurricular",
          headers: {
            "Content-Type": "application/json",
          });
      if (response.statusCode == 200) {
